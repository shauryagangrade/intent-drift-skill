...
class DependencyProvider(EvidenceProvider):
    ...
    def collect(self, context: Dict[str, Any]) -> List[Evidence]:
        ...
        install_commands = [cmd for cmd in recent_commands if any(install in cmd.lower() for install in ['pip install', 'npm install', 'yarn add', 'conda install', 'gem install', 'cargo add'])]
        remove_commands = [cmd for cmd in recent_commands if any(remove in cmd.lower() for remove in ['pip uninstall', 'npm uninstall', 'yarn remove', 'conda remove'])]
        ...
        if install_commands:
            ...
            # Remove common words
            common_words = {'pip', 'install', 'npm', 'add', 'yarn', 'conda', 'gem', 'cargo', 'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by'}
            ...
            if goal_keywords and install_keywords:
                ...
                overlap_ratio = overlap / min(len(goal_keywords), len(install_keywords)) if min(len(goal_keywords), len(install_keywords)) > 0 else 0
                evidence.append(
                    Evidence(
                        source=self.name,
        ...
        package_files = [f for f in edited_files if any(pkg in f.lower() for pkg in ['package.json', 'requirements.txt', 'setup.py', 'pom.xml', 'build.gradle', 'cargo.toml', 'go.mod'])]
        if package_files:
            evidence.append(
                Evidence(
                    source=self.name,
