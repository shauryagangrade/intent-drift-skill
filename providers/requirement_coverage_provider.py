...
class RequirementCoverageProvider(EvidenceProvider):
    ...
    def collect(self, context: Dict[str, Any]) -> List[Evidence]:
        ...
        coverage_percentage = (addressed / len(success_criteria)) * 100 if success_criteria else 100.0
        ...
        unrelated_work_indicators = ['refactor', 'cleanup', 'documentation', 'testing', 'optimization']
        plan_text_lower = ' '.join(current_steps).lower()
        unrelated_count = sum(1 for indicator in unrelated_work_indicators if indicator in plan_text_lower)
